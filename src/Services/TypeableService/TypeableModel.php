...
class TypeableModel
{
    ...
    private function setProperties()
    {
        ...
        foreach ($this->appends as $field => $type) {
            $this->properties[$field] = TypeableProperty::make($this->table, new TypeableDbColumn($field, $type));
        }

        foreach ($this->relations as $field => $relation) {
            $this->properties[$field] = TypeableProperty::make($this->class->table, new TypeableDbColumn($field, $relation->type), true);
        }

        if ($this->mediable) {
            $this->properties['mediable'] = TypeableProperty::make($this->table, new TypeableDbColumn('mediable', $this->mediable), true);
        }
    }
