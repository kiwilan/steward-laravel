...
function plugin(userOptions = {}) {
  return {
    ...
    async buildStart() {
      ...
      import_fs.default.copyFile(fullPath, `${opts.outputDir}/color-mode.js`, (err) => {
        if (err)
          throw err;
      });
    }
...
function run$1(config) {
  var _a;
  const { editor: editor2, from: from2, to, text: text2, rules, plugin: plugin2 } = config;
  const { view } = editor2;
  if (view.composing) {
    return false;
  }
